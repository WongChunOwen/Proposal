...
# Data Cleaning
As for data cleaning, we converted the date column to Date format, and extracted the year and month. The data was grouped by country, year, and month to calculate the average temperature, rounded to two decimal places. An initial check for NaN values was performed, displaying and removing rows related to "Antarctica" due to complete NaN values. Missing values were filled using a median calculation within a 5-year window for the same month. In addition, to smooth any remaining smaller gap NaN values and ensure continuity, a function from the zoo library is used. By applying linear interpolation and forward/backward filling, it ensures continuous and complete temperature records. The final values were then rounded to two decimal places. Thereafter, a final check for NaN values was conducted, and the imputed temperature data for Afghanistan in 1839 was filtered and displayed to validate the imputation process.
```{r, message = FALSE, warning = FALSE}
...
knitr::kable(head(rows_with_nan_initial), caption = "Rows with NA and NaN Values")
...
# Function to fill NaN values with median of 5-year window for the same month
...
monthly_avg_all_countries_median <- fill_with_window_month_median(monthly_avg_all_countries_initial)
...
monthly_avg_all_countries_filled <- fill_nan(monthly_avg_all_countries_median)

# Round the final dataframe to 2 decimal places
monthly_avg_all_countries_filled <- monthly_avg_all_countries_filled |>
  mutate(monthly_avg = round(monthly_avg, 2))

# Checking for remaining NaN values in the median imputed dataset
nan_check_filled <- sapply(monthly_avg_all_countries_filled, function(x) sum(is.na(x)))
print(nan_check_filled)

# Display rows with NaN values in the median imputed dataset
rows_with_nan_final <- monthly_avg_all_countries_filled[apply(monthly_avg_all_countries_filled, 1, function(row) any(is.na(row))), ]
print(rows_with_nan_final)

# Filter the data for Afghanistan in the year 1839 to check the imputed values
afghanistan <- monthly_avg_all_countries_filled |>
  filter(Country == "Afghanistan" & year == 1839)

# Display the filtered data for Afghanistan in 1839
knitr::kable(head(afghanistan), caption = "Filtered Data for Afghanistan in 1839", digits = 2)
```
...
joinedDT <-
  full_join(
    monthly_avg_all_countries_filled,
    country_codes,
    by = c("Country")
  )
...
monthly_avg_all_countries_filled$Country <- gsub("\\s*\\([^)]*\\)|\\s*\\[[^]]*\\]|\\s+Islands", "",monthly_avg_all_countries_filled$Country)
```
