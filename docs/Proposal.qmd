...
We converted the date column to Date format and extracted the year and month. The data was then grouped by country, year, and month to calculate the average temperature, rounded to two decimal places. An initial check for NaN values was performed, displaying and removing rows related to "Antarctica" due to complete NaN values. Missing values were filled using linear interpolation and forward/backward filling. Additionally, a median calculation within a 5-year window for the same month was applied to ensure continuous and complete temperature records. The final values were then rounded to two decimal places. A final check for NaN values was conducted, and the imputed temperature data for Afghanistan in 1839 was filtered and displayed to validate the imputation process.
